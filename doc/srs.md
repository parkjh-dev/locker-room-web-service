...
| ID | 요구사항 | 상세 |
|----|----------|------|
| UI-LAYOUT-001 | Public 레이아웃 | 헤더 + 컨텐츠 (사이드바 없음) |
| UI-LAYOUT-002 | Private 레이아웃 | 헤더 + 사이드바 + 컨텐츠 (SidebarLayout) |
| UI-LAYOUT-003 | Admin 레이아웃 | 관리자 전용 사이드바 + 컨텐츠 |
| UI-LAYOUT-004 | 헤더 (비로그인) | 로고, 로그인/회원가입 버튼 |
| UI-LAYOUT-005 | 헤더 (로그인) | 로고, 알림 아이콘(미읽음 배지), 유저 프로필 드롭다운 |
| UI-LAYOUT-006 | 사이드바 | 게시판 목록, 마이페이지/고객센터/요청 링크 |
